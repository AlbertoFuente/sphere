define(['text!../views/javaContent.html', 'utils', 'jquery'], function(html, utils, $) {
    'use strict';
    ...
    var _javaContent = function(posts) {
        var container = document.getElementById('container');

        if (!utils._isUnd(posts) && !utils._isUnd(html)) {
            container.innerHTML = html;
            var miniPanel = document.getElementById('javaMiniPanel');

            if (!utils._isUnd(miniPanel)) {
                var title = document.createElement('h5'),
                    link = document.createElement('a'),
                    postsContainer = document.createElement('div'),
                    pContainer = document.getElementById('javaContentPanel');

                title.innerHTML = posts.title;
                utils._setAttr(link, 'href', posts.link.slice(0, -3));
                ...
                if (!utils._isUnd(posts.entries) && posts.entries.length > 0) {
                    ...
                    for (i; i < entries; i++) {
                        ...
                        utils._appendArr(subDiv, [subLink, subText]);
